...
//        ossClient = new OSSClient(endpoint, accessKeyId, accessKeySecret);
    ...
    @Override
    public String fileUplodnew(String fileStr,String filename,String fileUrl) {
        String url="https://yueguoyu.oss-cn-beijing.aliyuncs.com/"+fileUrl;
        OSSClient ossClient = new OSSClient(endpoint, accessKeyId, accessKeySecret);
        // 上传文件。<yourLocalFile>由本地文件路径加文件名包括后缀组成，例如/users/local/myfile.txt。
        ossClient.putObject("yueguoyu", fileUrl+filename, new File(fileStr));
        // 关闭OSSClient。
        ossClient.shutdown();
        return url+filename;
    }
